...
module 內底提供一个函式 `tlst()`，但是有兩種呼叫方式，也就是會使接受兩種
...
`tlst()` 就是用來將台羅數字版轉做調號版个函式，有兩種呼叫介面：
...
1. `tlst($match)` 
   ...
   迄个變數）。了後將 `$match` 擲予 `tlst()` 函式做轉換，就會得著
...
2. `tlst($str)`
   ...
   規篇台羅文章嘛無差，擲予 tlst 進行轉換，來紡出正式版个調號式台羅。
...
二、`tlst.raku` 命令列程式，是將數字式台羅純文字檔轉換做調號式台羅純文字檔个
軟體家私。

### 用法：

1. 將 `TLconv.rakumod` 佮 `tlst.raku` 囥佇仝一个工作目錄內底，欲用進前，
   愛先確認 `tlst.raku` 个檔案存取模式是「可執行」--个。了後執行：
   
   ```
   ./tlst.raku "數字式台羅文字檔名.txt"
   ```
2. 程式會將轉換結果直接顯示佇螢幕畫面，這會使得用「輸出重新導向」个方式，
   將結果存檔起來：
   
   ```
   ./tlst.raku "數字式台羅文字檔名.txt" > "調號式台羅文字檔名.txt"
   ```
   注意：
   ...
   * 佇 Windows CMD（所謂 DOS 視窗）/ Powershell 內底使用，愛扑：
   ...
   ```
   raku tlst.raku "數字式台羅文字檔名.txt" > "調號式台羅文字檔名.txt"
   ```
...
1. 將本說明檔閣有一个 module 檔 `TLconv.rakumod` 佮程式檔 `tlst.raku` 掠落來，
