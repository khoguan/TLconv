...
```
   ...
   o「輸入檔名」（來源檔）佮「輸出檔名」（結果檔）著愛號無仝款名，才袂未曾未
     就將來源檔清空去。
   o 佇 Windows CMD（所謂 DOS 視窗）/ Powershell 內底使用，愛扑：
```
...
0. 將本說明檔閣有一个 module 檔 `TLconv.rakumod` 佮程式檔 `tls-tlt.raku` 掠落來，
...
2. 去「開始功能表->所有程式->Windows PowerShell->Windows PowerShell」揤 mouse 右鍵，
...
6. 以後欲執行台羅轉換个指令，只要點兩下即个捷徑，就會出現 PowerShell 視窗予咱扑指令，可比講：
```
     raku tls-tlt.raku "數字式台羅文字檔名.txt" > "調號式台羅文字檔名.txt"
```
